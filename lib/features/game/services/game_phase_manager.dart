...
void handleRoundResultPhase(
  GameState state, 
  Map<String, dynamic> data
) {
  // Debug için veriyi yazdır
  print("Round Result Phase Data: $data");
  print("State before update: isGreenTeam=${state.isGreenTeam}, playerScore=${state.playerScore}, opponentScore=${state.opponentScore}");
  ...
  print("Score update - data values: player1Wins=${data['player1Wins']}, player2Wins=${data['player2Wins']}");
  print("Score update - calculated: playerWins=$playerWins, opponentWins=$opponentWins");
  
  // Rakibin hamlesini belirle
  String? opponentMove = null;
  if (movesData != null) {
    opponentMove = state.isGreenTeam ? movesData['player2'] : movesData['player1'];
  }
  ...
  print("Final state: playerScore=${updatedState.playerScore}, opponentScore=${updatedState.opponentScore}");
  ...
  void handleJokerSelectPhase(
    ...
    if (availableJokersData != null) {
      final playerJokersData = state.isGreenTeam 
      ...
      if (playerJokersData != null) {
        ...
        updatedJokers[JokerType.blind] = playerJokersData['hook'] ?? 0; // Server'da 'hook'
  ...
  void handleJokerRevealPhase(
    ...
    if (nextRoundBlockedData != null) {
      if (state.isGreenTeam) {
        nextRoundBlocked = List<String>.from(nextRoundBlockedData['player1'] ?? []);
      } else {
        nextRoundBlocked = List<String>.from(nextRoundBlockedData['player2'] ?? []);
      }
    }
    ...
    final updatedState = _jokerHandler.applyJokerEffects(
      state: state,
  ...
  GameState updatedState = state.copyWith(
    ...
    opponentScore: opponentWins,
    betMultiplier: data['betMultiplier']?.toDouble() ?? state.betMultiplier,
    jokerUsageStatus: 'none', // Round sonu ile joker kullanım durumunu sıfırla
